...
public class XssHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public String getParameter(String name) {
        String value = super.getParameter(xssEncode(name.trim()));
        if (value != null) {
            value = xssEncode(value.trim());
        }
        return value;
    ...
    public String[] getParameterValues(String name) {
        String[] parameters = super.getParameterValues(name.trim());
        if (parameters == null || parameters.length == 0) {
            return null;
        }
        for (int i = 0; i < parameters.length; i++) {
            parameters[i] = xssEncode(parameters[i].trim());
        }
        return parameters;
    }
    ...
    public String getHeader(String name) {
        ...
        String value = super.getHeader(xssEncode(name.trim()));
        if (value != null) {
            value = xssEncode(value.trim());
        }
        return value;
